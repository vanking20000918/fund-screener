...
4. 计算 **6 维度软性评分**(业绩稳定性、熊市表现、任职年限、卡玛比率、风格一致性、规模适中度)
5. 加权排序,输出 Top 20 推荐
...
│   ├── backtest.py                 # PIT 回测验证(选历史时点重跑评分对比基准)
│   ├── report_generator.py         # HTML + Excel 报告生成
│   └── mail_sender.py              # QQ 邮箱 SMTP 发送
├── main.py                         # 月度报告主入口
├── backtest.py                     # 回测脚本入口: python backtest.py --date 2024-01-01
...
# 评分权重(总和=1.0, 评分卡 v2.0)
SCORE_WEIGHTS = {
    'stability': 0.28,    # 业绩稳定性: 近3年全市场分位
    'bear_perf': 0.20,    # 熊市相对表现: 历轮熊市回撤候选池分位 + 经验加成
    'tenure':    0.15,    # 经理任职年限: 分段线性 3→55, 7→85, 15+→100
    'framework': 0.15,    # 投资框架: 卡玛比率(年化收益/最大回撤)
    'style':     0.12,    # 风格一致性: 行业配置相似度(eastmoney 用波动率代理)
